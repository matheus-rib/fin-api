...
export class TransactionsController {
  constructor (private transactionsProvider: TransactionsProvider) {
  ...
  async index (@Query() query: QueryString<Transaction>): Promise<PaginatedList<Transaction>> {
    return this.transactionsProvider.index(query)
  }
  ...
  async store (@Body() createTransactionDTO: CreateTransactionDTO): Promise<Transaction> {
    return this.transactionsProvider.store(createTransactionDTO)
  }
}
